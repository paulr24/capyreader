...
class AISummarizerService(
    ...
    suspend fun summarize(article: Article): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            val summary = when (provider) {
                AIProvider.GEMINI -> callGemini(prompt = prompt, model = aiOptions.geminiModel.get(), apiKey = aiOptions.geminiApiKey.get())
    ...
    companion object {
        ...
        private const val MAX_ARTICLE_TEXT_LENGTH = 30000
    }
}
